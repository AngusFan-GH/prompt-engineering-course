...
2. [核心概念定义](#1-核心概念定义)
3. [关键技术要素](#2-关键技术要素)
4. [主流技巧和方法论](#3-主流技巧和方法论)
5. [实际代码案例](#4-实际代码案例)
6. [常见陷阱与最佳实践](#5-常见陷阱与最佳实践)
7. [教学场景化案例](#6-教学场景化案例)
8. [附录与模板](#附录常用prompt模板)
9. [练习题](#练习题)
10. [参考资源](#参考资源)
...
**提示词工程（Prompt Engineering）** 是优化与大语言模型（LLM）交互输入的技术和方法论，目的是获得更准确、更有用的输出。
...
|---------|------|---------|
| 基础 | 简单问答 | 60% |
| 进阶 | 结构化输出 + Few-shot | 80% |
| 高级 | CoT + ReAct + 链式调用 | 95% |
...
print(f"英文 '{text_en}' 估算tokens: {estimate_tokens(text_en)}")  # ≈ 3
print(f"中文 '{text_cn}' 估算tokens: {estimate_tokens(text_cn)}")  # ≈ 5
```
...
│                                                            │
│  Temperature (温度参数)                                     │
│  ├── 0.0 - 0.3: 确定性强，适合代码生成、事实问答            │
│  ├── 0.4 - 0.6: 平衡模式，适合通用对话                    │
│  └── 0.7 - 1.0: 创意性强，适合写作、头脑风暴               │
│                                                            │
│  Top-p (核采样)                                            │
│  ├── 控制输出多样性的采样策略                               │
│  ├── 值越小，输出越保守；值越大，输出越多样                │
...
| 模型 | 上下文窗口 | 适用场景 |
|------|-----------|---------|
| GPT-4 | 128K tokens | 长文档分析、多轮对话 |
| Claude 3 | 200K tokens | 长文本处理 |
| Gemini 1.5 | 1M tokens | 超长文档处理 |
...
│      ▼                                                               │
│   ┌─────────────────────────────────────────┐                        │
│   │  步骤1: Summarize (摘要)                │ ◄── 子任务1           │
│   └─────────────────┬───────────────────────┘                        │
│                     │                                                 │
│                     ▼                                                 │
│   ┌─────────────────────────────────────────┐                        │
│   │  步骤2: Extract Keywords (关键词)       │ ◄── 子任务2           │
│   └─────────────────┬───────────────────────┘                        │
│                     │                                                 │
│                     ▼                                                 │
│   ┌─────────────────────────────────────────┐                        │
│   │  步骤3: Generate Tags (生成标签)        │ ◄── 子任务3           │
│   └─────────────────┬───────────────────────┘                        │
│                     │                                                 │
│                     ▼                                                 │
│               最终输出                                                │
...
def self_consistency_voting(llm_client, prompt: str, n_samples: int = 5) -> str:
    """
...
│   ├── 02_role_prompting.py    # 角色设定
│   ├── 03_few_shot.py          # 少样本学习
│   ├── 04_cot.py               # 思维链
│   ├── 05_react.py             # ReAct模式
│   ├── 06_advanced_techniques.py # 高级技巧
│   ├── 07_rag_qa_system.py     # RAG问答系统
│   └── 08_teaching_cases.py    # 教学案例
├── utils/
│   ├── __init__.py
...
```python
TRANSLATION_TEMPLATE = """请将以下{source_lang}文本翻译为{target_lang}。
...
### 练习4：Chain Chaining实践

设计一个文章摘要生成流水线，包含：关键词提取 → 摘要生成 → 标签生成
